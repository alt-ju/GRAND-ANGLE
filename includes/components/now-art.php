...
$sqlExpoNow = "SELECT exposition.libelle_Exposition
FROM exposition
WHERE exposition.Date_Debut <= DATE_ADD(CURRENT_DATE(), INTERVAL 3 DAY) AND CURRENT_DATE() <= exposition.Date_Fin";
$requeteExpoNow = $db->query($sqlExpoNow);
$expositionsNow = $requeteExpoNow->fetchAll(PDO::FETCH_ASSOC);
...
<div class="art-content-now">
    <?php forEach($expositionsNow as $expositionNow): ?>
    <div class="expo-content-now">
        <div class="title-art-content-now-expo">
                <h2><?= $expositionNow["libelle_Exposition"];?></h2>
        </div>
